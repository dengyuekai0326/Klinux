...
from ball_control.modes import ModeController
from ball_control.protocol import TaskCommand
...
class ModeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.controller = ModeController(
            config.modes, config.calibration.half_length_cm
        )

    def test_mode2_sequence(self):
        state = self.controller.start(TaskCommand(2, 0.0), 0.0)
        self.assertEqual((state.mode, state.step, state.target_cm), (2, 0, 0.0))
        self.controller.update(0.10, 0.0)
        state, changed = self.controller.update(0.41, 0.0)
        self.assertTrue(changed)
        self.assertEqual((state.step, state.target_cm), (1, 5.0))
        self.controller.update(0.50, 5.0)
        state, changed = self.controller.update(1.01, 5.0)
        self.assertTrue(changed)
        self.assertEqual((state.step, state.target_cm), (2, -5.0))
        self.controller.update(1.10, -5.0)
        state, changed = self.controller.update(2.11, -5.0)
        self.assertTrue(changed)
        self.assertTrue(state.completed)

    def test_mode3_target_is_clamped(self):
        state = self.controller.start(TaskCommand(3, 20.0), 1.0)
        self.assertEqual(state.target_cm, 12.5)
